...
public interface DesignationSkillRepository extends JpaRepository<DesignationSkill, Long> {
  ...
  List<DesignationSkill> findByDesignationAndDifficultyLevel(String designation,
  ...
  @Query(
      "SELECT d.id, d.designation, d.skillName, d.skillDescription, COUNT(DISTINCT c.id) as companyCount "
          +
  ...
  @Query(
      "SELECT c.id, c.name, c.logoUrl, c.website, c.headquarters, COUNT(j.id) as activeJobCount " +
