...
import com.tymbl.interview.repository.CompanyInterviewQuestionRepository;
import com.tymbl.interview.repository.DesignationSkillRepository;
import com.tymbl.interview.repository.GeneralInterviewQuestionRepository;
import com.tymbl.interview.repository.QuestionGenerationQueueRepository;
...
public class InterviewPreparationService {
  ...
  public List<DesignationSkillDTO> getTopicsByDesignationAndDifficulty(String designation,
      DesignationSkill.DifficultyLevel difficultyLevel) {
    log.info("Fetching topics for designation: {} with difficulty: {}", designation,
        difficultyLevel);
    return designationSkillRepository.findByDesignationAndDifficultyLevel(designation,
            difficultyLevel)
        .stream()
        .map(DesignationSkillDTO::fromEntity)
        .collect(Collectors.toList());
  ...
  public List<DesignationSkillDTO> getTopicsByDesignationAndCategory(String designation,
      String category) {
  ...
  public List<InterviewQuestionDTO> getGeneralQuestionsByDesignationAndTopic(String designation,
      String topicName) {
    log.info("Fetching general questions for designation: {} and topic: {}", designation,
        topicName);
  ...
  public Page<InterviewQuestionDTO> getGeneralQuestionsByDesignationAndTopicPaginated(
      String designation, String topicName, int page, int size) {
    log.info("Fetching paginated general questions for designation: {} and topic: {}", designation,
        topicName);
  ...
  public List<InterviewQuestionDTO> getCompanyQuestionsByCompanyAndDesignation(String companyName,
      String designation) {
    log.info("Fetching company questions for company: {} and designation: {}", companyName,
        designation);
    return companyInterviewQuestionRepository.findByCompanyNameAndDesignation(companyName,
            designation)
        .stream()
  ...
  public List<InterviewQuestionDTO> getCompanyQuestionsByCompanyAndDesignationAndTopic(
      String companyName, String designation, String topicName) {
    log.info("Fetching company questions for company: {}, designation: {} and topic: {}",
        companyName, designation, topicName);
    return companyInterviewQuestionRepository.findByCompanyNameAndDesignationAndTopicName(
            companyName, designation, topicName)
        .stream()
        .map(InterviewQuestionDTO::fromCompanyEntity)
        .collect(Collectors.toList());
  ...
  public Page<InterviewQuestionDTO> getCompanyQuestionsByCompanyAndDesignationAndTopicPaginated(
      String companyName, String designation, String topicName, int page, int size) {
    log.info("Fetching paginated company questions for company: {}, designation: {} and topic: {}",
        companyName, designation, topicName);
    Pageable pageable = PageRequest.of(page, size);
    return companyInterviewQuestionRepository.findByCompanyNameAndDesignationAndTopicName(
            companyName, designation, topicName, pageable)
        .map(InterviewQuestionDTO::fromCompanyEntity);
  }

  public List<String> getTopicsWithCompanyQuestions(String companyName, String designation) {
    log.info("Fetching topics with company questions for company: {} and designation: {}",
        companyName, designation);
    return companyInterviewQuestionRepository.findTopicsByCompanyAndDesignation(companyName,
        designation);
  }
  ...
  public Long requestQuestionGeneration(QuestionGenerationRequestDTO request) {
    ...
    QuestionGenerationQueue.RequestType requestType = QuestionGenerationQueue.RequestType.valueOf(
        request.getRequestType().toUpperCase());
    QuestionGenerationQueue.DifficultyLevel difficultyLevel = QuestionGenerationQueue.DifficultyLevel.valueOf(
  ...
  private List<Map<String, Object>> generateCompanySpecificQuestions(
      QuestionGenerationQueue queueEntry) {
    List<Map<String, Object>> allQuestions = new ArrayList<>();
  ...
  private void saveGeneratedQuestions(QuestionGenerationQueue queueEntry,
    ...
    for (Map<String, Object> questionData : questions) {
      if (queueEntry.getRequestType() == QuestionGenerationQueue.RequestType.GENERAL) {
        GeneralInterviewQuestion question = GeneralInterviewQuestion.builder()
            ...
            .difficultyLevel(GeneralInterviewQuestion.DifficultyLevel.valueOf(
                (String) questionData.get("difficulty_level")))
            .questionType(GeneralInterviewQuestion.QuestionType.valueOf(
                (String) questionData.get("question_type")))
        ...
        CompanyInterviewQuestion question = CompanyInterviewQuestion.builder()
            ...
            .questionText((String) questionData.get("question_text"))
            .answerText((String) questionData.get("answer_text"))
            .difficultyLevel(CompanyInterviewQuestion.DifficultyLevel.valueOf(
                (String) questionData.get("difficulty_level")))
            .questionType(CompanyInterviewQuestion.QuestionType.valueOf(
                (String) questionData.get("question_type")))
  ...
  public Map<String, Object> getStatistics(String designation) {
    ...
    stats.put("topics_by_difficulty", topics.stream()
        .collect(Collectors.groupingBy(topic -> topic.getDifficultyLevel().name(),
            Collectors.counting())));

    // General questions statistics
    List<GeneralInterviewQuestion> generalQuestions = generalInterviewQuestionRepository.findByDesignation(
        designation);
    stats.put("total_general_questions", generalQuestions.size());
    stats.put("general_questions_by_topic", generalQuestions.stream()
        .collect(
            Collectors.groupingBy(GeneralInterviewQuestion::getTopicName, Collectors.counting())));

    // Company questions statistics
    List<String> companies = companyInterviewQuestionRepository.findCompaniesByDesignation(
        designation);
  ...
  public List<Map<String, Object>> generateTopicsForDesignation(String designationName) {
    ...
    List<Map<String, Object>> generatedTopics = geminiService.generateTopicsForDesignation(
        designationName);
    ...
    log.info("Successfully generated {} topics for designation: {}", generatedTopics.size(),
        designationName);
  ...
  @Transactional
  public List<DesignationSkillDTO> saveGeneratedTopics(String designationName,
    ...
    for (Map<String, Object> topicData : topics) {
      try {
        DesignationSkill topic = DesignationSkill.builder()
            ...
            .difficultyLevel(DesignationSkill.DifficultyLevel.valueOf(
                ((String) topicData.get("difficulty_level")).toUpperCase()))
    ...
    log.info("Successfully saved {} topics for designation: {}", savedTopics.size(),
        designationName);
    return savedTopics;
  }
