...
public interface PendingContentRepository extends JpaRepository<PendingContent, Long> {
   ...
   */
  java.util.List<PendingContent> findByProcessedAtIsNullOrProcessedAtBefore(
      java.time.LocalDateTime cutoffTime);

  /**
   * Find pending content by entity type and source table
   */
  java.util.List<PendingContent> findByEntityTypeAndSourceTable(
      PendingContent.EntityType entityType, String sourceTable);
