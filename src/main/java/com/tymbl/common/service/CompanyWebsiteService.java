...
public class CompanyWebsiteService {
    ...
    public Map<String, Object> fetchWebsiteForCompany(Company company) {
        ...
        try {
            ...
            log.info("Fetched website URL '{}' for company: {} (ID: {})", website, company.getName(), company.getId());
            
        } catch (Exception e) {
            log.error("Error fetching website for company: {} (ID: {})", company.getName(), company.getId(), e);
            company.setWebsiteFetched(2); // Mark as failed
            companyRepository.save(company);
    ...
    public Map<String, Object> fetchWebsitesForAllCompaniesInBatches() {
        ...
        while (true) {
            ...
            Page<Company> companyPage = companyRepository.findByWebsiteFetched(0, PageRequest.of(pageNumber, batchSize));
            
            List<Company> companies = companyPage.getContent();
            
            if (companies.isEmpty()) {
                log.info("No more companies to process for website fetching. Completed at page: {}", pageNumber);
                break;
            }
            
            log.info("Processing website fetching batch {} with {} companies", pageNumber + 1, companies.size());
            ...
            for (Company company : companies) {
                try {
                    ...
                    if ((Boolean) result.get("success")) {
                        ...
                        log.info("Successfully fetched website for company: {} (ID: {})", company.getName(), company.getId());
                    } else {
                        totalErrors++;
                        log.error("Failed to fetch website for company: {} (ID: {})", company.getName(), company.getId());
                    }
        ...
        log.info("Completed website fetching using database pagination. Total processed: {}, Websites fetched: {}, Errors: {}", 
    ...
    private String generateWebsiteForCompany(String companyName) {
        try {
            ...
            ResponseEntity<String> response = aiRestService.callGeminiAPI(requestBody, "Website Generation for " + companyName);
