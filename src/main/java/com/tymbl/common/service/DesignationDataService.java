...
public class DesignationDataService {
  ...
  /**
   * Clean up duplicate designations using the same logic as the SQL query: DELETE FROM designations
   * WHERE id NOT IN (SELECT id FROM (SELECT MIN(id) AS id FROM designations GROUP BY CASE WHEN
   * RIGHT(name, 2) = '"]' THEN LEFT(name, LENGTH(name) - 2) ELSE name END) AS t);
   */
  @Transactional
    ...
    try {
      ...
      Map<String, List<Designation>> designationsByNormalizedName = allDesignations.stream()
          .collect(Collectors.groupingBy(designation ->
              DesignationNameCleaner.getNormalizedNameForDuplicateDetection(
                  designation.getName())));
   ...
   * removes duplicates. This method: 1. Cleans all designation names using DesignationNameCleaner
   * 2. Removes duplicates based on cleaned names 3. Keeps the designation with minimum ID for each
   * group
   */
  @Transactional
