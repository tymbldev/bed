...
public class NotificationService {
   ...
   */
  @Transactional
  public void createReferralApplicationNotification(JobReferrer jobReferrer,
    ...
    try {
      ...
      String message = String.format(
          "Someone applied for the referral you posted for %s position at %s", jobTitle,
          companyName);
      ...
      log.info("Created referral application notification for user: {}",
          jobReferrer.getUser().getId());
    } catch (Exception e) {
  ...
  public void createApplicationShortlistedNotification(JobApplication application) {
    try {
      ...
      String message = String.format(
          "Congratulations! Your application for %s position at %s has been shortlisted.", jobTitle,
          companyName);
      ...
      notificationRepository.save(notification);
      log.info("Created application shortlisted notification for user: {}",
          application.getApplicantId());
    } catch (Exception e) {
  ...
  public void createApplicationStatusNotification(JobApplication application, String status) {
    try {
      ...
      String message = String.format("Your application for %s position at %s has been %s.",
          jobTitle, companyName, status.toLowerCase());
      ...
      notificationRepository.save(notification);
      log.info("Created application status notification for user: {} with status: {}",
          application.getApplicantId(), status);
    } catch (Exception e) {
  ...
  public List<NotificationDTO> getRecentNotifications(Long userId) {
    ...
    List<Notification> notifications = notificationRepository.findRecentNotificationsByUserId(
        userId, sevenDaysAgo);
  ...
  public void markAllAsRead(Long userId) {
    List<Notification> unreadNotifications = notificationRepository.findByUserIdAndIsReadFalseOrderByCreatedAtDesc(
        userId);
