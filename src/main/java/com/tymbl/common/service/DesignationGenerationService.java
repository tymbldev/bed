...
public class DesignationGenerationService {
    ...
    for (Department department : departments) {
      ...
      try {
        ...
        if ((Boolean) departmentProcessResult.get("success")) {
          ...
          departmentResult.put("designationsGenerated",
              departmentProcessResult.get("designationsGenerated"));
          departmentResult.put("designationsSaved",
              departmentProcessResult.get("designationsSaved"));
  ...
  public Map<String, Object> processDepartmentInTransaction(Department department) {
    ...
    try {
      List<String> designations = generateDesignationsForDepartment(department.getName(),
  ...
  private String buildDesignationGenerationPrompt(String departmentName,
    ...
    return String.format(
        ...
        departmentName,
        departmentDescription != null ? departmentDescription : "No description available",
        departmentName
    );
  }
  ...
  private int saveDesignationsToDatabase(List<String> designations, Department department) {
    ...
    log.info("Starting to save {} designations for department: {} ({})", designations.size(),
        department.getName(), department.getId());

    for (String rawDesignationName : designations) {
      try {
        // Clean and validate the designation name
        String cleanedDesignationName = DesignationNameCleaner.cleanAndValidateDesignationName(
            rawDesignationName);
        if (cleanedDesignationName == null) {
          log.warn("Skipping invalid designation name: '{}' for department: {}", rawDesignationName,
              department.getName());
          skippedCount++;
          continue;
        }
        ...
        if (!designationExists) {
          ...
          savedCount++;
          log.info("Saved designation: {} for department: {}", cleanedDesignationName,
              department.getName());
        } else {
          log.info("Designation already exists: {} for department: {}", cleanedDesignationName,
              department.getName());
        }
      } catch (Exception e) {
        errorCount++;
        log.error("Error saving designation: {} for department: {}, Error: {}", rawDesignationName,
            department.getName(), e.getMessage());
      }
    }

    log.info(
        "Designation save completed for department: {} ({}). Saved: {}, Skipped: {}, Errors: {}",
  ...
  /**
   * Process a single department for designation generation This method processes one department in
   * its own transaction
   */
  @Transactional
