...
public class CompanyDataService {
  ...
  /**
   * Clean up duplicate companies using the same logic as the SQL query: DELETE FROM companies WHERE
   * id NOT IN (SELECT id FROM (SELECT MIN(id) AS id FROM companies GROUP BY CASE WHEN RIGHT(name,
   * 2) = '"]' THEN LEFT(name, LENGTH(name) - 2) ELSE name END) AS t);
   */
  @Transactional
    ...
    try {
      ...
      for (Map.Entry<String, List<Company>> entry : companiesByNormalizedName.entrySet()) {
        ...
        if (companies.size() > 1) {
          ...
          results.add("Removed " + toRemove.size() + " duplicate(s) for normalized name: "
              + normalizedName);
        }
      }
  ...
  /**
   * Comprehensive cleanup method that applies all the cleaning logic from the SQL queries and then
   * removes duplicates. This method: 1. Cleans all company names using CompanyNameCleaner 2.
   * Removes duplicates based on cleaned names 3. Keeps the company with minimum ID for each group
   */
  @Transactional
  ...
  public List<String> loadBasicCompanyData() {
    ...
    try (InputStream inputStream = getClass().getClassLoader()
        .getResourceAsStream("db/companies_basic.txt");
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (fields.length >= 11) {
          try {
            ...
            String errorMsg =
                "Error processing line: " + line.substring(0, Math.min(50, line.length()))
                    + "... Error: " + e.getMessage();
            results.add(errorMsg);
          }
        } else {
          results.add("Invalid line format (expected 11 fields): " + line.substring(0,
              Math.min(50, line.length())) + "...");
        }
      }
    } catch (IOException e) {
  ...
  public List<String> updateDetailedCompanyData() {
    ...
    try (InputStream inputStream = getClass().getClassLoader()
        .getResourceAsStream("db/companies_detailed.txt");
        BufferedReader reader = new BufferedReader(
            new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        if (fields.length >= 5) {
          try {
            ...
            String errorMsg = "Error processing detailed data line: " + line.substring(0,
                Math.min(50, line.length())) + "... Error: " + e.getMessage();
            results.add(errorMsg);
          }
        } else {
          results.add("Invalid detailed data line format (expected 5 fields): " + line.substring(0,
              Math.min(50, line.length())) + "...");
        }
      }
    } catch (IOException e) {
    ...
    try {
      ...
      if (existingCompanyByName.isPresent()) {
        ...
        if (existingCompanyById.isPresent()) {
          ...
          updateCompanyBasicFields(company, cleanedName, description, website, logoUrl,
              headquarters,
              companySize, specialties, linkedinUrl, careersUrl);
          companyRepository.save(company);
      ...
      if (e.getCause() instanceof org.hibernate.exception.ConstraintViolationException) {
        errorMessage =
            "Database constraint violation for company data: " + e.getCause().getMessage();
      }
      return errorMessage;
    }
  }
  ...
  private String processDetailedCompanyData(String[] fields) {
    try {
      ...
      if (existingCompany.isPresent()) {
        ...
        if ((companyContent.getAboutUsOriginal() == null || companyContent.getAboutUsOriginal()
            .trim().isEmpty()) && aboutUs != null && !aboutUs.trim().isEmpty()) {
        ...
        if ((companyContent.getCultureOriginal() == null || companyContent.getCultureOriginal()
            .trim().isEmpty()) && culture != null && !culture.trim().isEmpty()) {
        ...
        if (updated) {
          ...
          return "No updates needed for company: " + company.getName()
      ...
      if (e.getCause() instanceof org.hibernate.exception.ConstraintViolationException) {
        errorMessage =
            "Database constraint violation for detailed company data: " + e.getCause().getMessage();
      }
      return errorMessage;
    }
  }

  private void updateCompanyBasicFields(Company company, String name, String description,
      String website,
