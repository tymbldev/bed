...
public class FirebaseNotificationService {
    ...
    public boolean sendNotificationToDevice(String deviceToken, String title, String message, Map<String, Object> data) {
        ...
        try {
            ...
            ResponseEntity<String> response = restTemplate.postForEntity(firebaseApiUrl, request, String.class);
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                ...
                if (success) {
                    ...
                    log.error("Failed to send notification to device: {}. Response: {}", deviceToken, response.getBody());
                    return false;
                }
            } else {
        ...
        try {
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                ...
                if (success) {
                    ...
                    log.error("Failed to send notification to topic: {}. Response: {}", topic, response.getBody());
                    return false;
                }
            } else {
                log.error("Firebase API error: {} - {}", response.getStatusCode(), response.getBody());
    ...
    public boolean sendNotification(Notification notification, String deviceToken) {
        ...
        try {
            ...
            data.put("relatedEntityId", notification.getRelatedEntityId() != null ? notification.getRelatedEntityId().toString() : "");
            data.put("relatedEntityType", notification.getRelatedEntityType() != null ? notification.getRelatedEntityType().toString() : "");
            data.put("click_action", "FLUTTER_NOTIFICATION_CLICK");
            
            return sendNotificationToDevice(deviceToken, notification.getTitle(), notification.getMessage(), data);
        } catch (Exception e) {
