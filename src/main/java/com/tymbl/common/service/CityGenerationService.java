...
public class CityGenerationService {
  ...
  private List<String> generateCitiesForCountry(String countryName) {
    try {
      ...
      ResponseEntity<String> response = aiRestService.callGeminiAPI(requestBody,
          "City Generation for " + countryName);
  ...
  private int saveCitiesToDatabase(List<String> cities, Country country) {
    ...
    log.info("Starting to save {} cities for country: {} ({})", cities.size(), country.getName(),
        country.getCode());
    ...
    for (String cityName : cities) {
      try {
        ...
        errorCount++;
        log.error("Error saving city: {} for country: {}, Error: {}", cityName, country.getName(),
            e.getMessage());
      }
    }
  ...
  /**
   * Process a single country for city generation This method processes one country in its own
   * transaction
   */
  @Transactional
  ...
  /**
   * Reset cities processed flag for all countries This allows reprocessing of city generation for
   * all countries
   */
  @Transactional
