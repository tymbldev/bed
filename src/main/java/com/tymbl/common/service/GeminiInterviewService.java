...
public class GeminiInterviewService {
    ...
    private String buildSimilarCompaniesPrompt(String companyName, String industry, String description) {
        return String.format(
            ...
            "Return ONLY a JSON array of strings with the similar company names. " +
            ...
            if (candidates != null && candidates.isArray() && candidates.size() > 0) {
                ...
                if (content != null) {
                    ...
                    if (parts != null && parts.isArray() && parts.size() > 0) {
                        ...
                        String jsonText = extractJsonFromText(generatedText);
                        if (jsonText != null) {
                            JsonNode companiesArray = objectMapper.readTree(jsonText);
                            if (companiesArray.isArray()) {
                                List<String> companies = new ArrayList<>();
                                for (JsonNode companyNode : companiesArray) {
                                    if (companyNode.isTextual()) {
                                        companies.add(companyNode.asText().trim());
                                    }
                                }
                                return companies;
                            }
    ...
    private String buildSimilarDesignationsPrompt(String designationName) {
        return String.format(
            ...
            "Return ONLY a JSON array of strings with the similar designation names. " +
            "Do not include any explanations, comments, or additional text. " +
            ...
            if (candidates != null && candidates.isArray() && candidates.size() > 0) {
                ...
                if (content != null) {
                    ...
                    if (parts != null && parts.isArray() && parts.size() > 0) {
                        ...
                        String jsonText = extractJsonFromText(generatedText);
                        if (jsonText != null) {
                            JsonNode designationsArray = objectMapper.readTree(jsonText);
                            if (designationsArray.isArray()) {
                                List<String> designations = new ArrayList<>();
                                for (JsonNode designationNode : designationsArray) {
                                    if (designationNode.isTextual()) {
                                        designations.add(designationNode.asText().trim());
                                    }
                                }
                                return designations;
                            }
