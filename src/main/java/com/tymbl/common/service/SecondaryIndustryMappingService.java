...
public class SecondaryIndustryMappingService {
  ...
  public Map<String, Object> processSecondaryIndustries() {
    ...
    try {
      ...
      for (String industryName : allSecondaryIndustries) {
        ...
        try {
          ...
          if (mappedName != null && !mappedName.trim().isEmpty()) {
            ...
            log.info("Processed industry '{}' → mapped to '{}' (ID: {})", industryName, mappedName,
                mappedId);
          } else {
            industryResult.put("success", false);
      ...
      log.info(
          "Secondary industry processing completed. Total: {}, Processed: {}, Errors: {}, Skipped: {}",
  ...
  public Map<String, Object> processSingleSecondaryIndustry(String industryName) {
    ...
    try {
      ...
      // Check if already processed
      if (secondaryIndustryMappingRepository.existsByName(industryName)) {
        SecondaryIndustryMapping existing = secondaryIndustryMappingRepository.findByName(
            industryName).orElse(null);
      ...
      if (mappedName != null && !mappedName.trim().isEmpty()) {
        ...
        log.info("Successfully processed industry '{}' → mapped to '{}' (ID: {})", industryName,
            mappedName, mappedId);
      } else {
        result.put("success", false);
  ...
  public Map<String, Object> resetProcessedFlag() {
    ...
    try {
      ...
      result.put("success", true);
      result.put("message",
          "Processed flag reset successfully for all secondary industry mappings");
  ...
  private List<String> getAllUniqueSecondaryIndustries() {
    ...
    List<String> allSecondaryIndustries = companyRepository.findAll().stream()
        .filter(
            company -> company.getSecondaryIndustries() != null && !company.getSecondaryIndustries()
                .trim().isEmpty())
    ...
    log.info("Extracted {} unique secondary industries from companies table",
        allSecondaryIndustries.size());
  ...
  private String generateMappedName(String industryName) {
    try {
      ...
      ResponseEntity<String> response = aiRestService.callGeminiAPI(requestBody,
          "Secondary Industry Mapping for " + industryName);
  ...
  private Long getOrCreateMappedId(String mappedName) {
    ...
    List<SecondaryIndustryMapping> existingMappings = secondaryIndustryMappingRepository.findByMappedName(
        mappedName);
