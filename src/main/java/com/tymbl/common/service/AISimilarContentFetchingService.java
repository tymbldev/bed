...
public class AISimilarContentFetchingService {
  ...
  public List<String> generateSimilarDesignations(String designationName) {
    try {
      ...
      ResponseEntity<String> response = aiRestService.callGeminiAPI(requestBody,
          "Similar Designations for " + designationName);
      List<String> similarDesignations = parseSimilarDesignationsResponse(response.getBody());
      log.info("[Gemini] Parsed {} similar designations for: {}", similarDesignations.size(),
          designationName);
      ...
      if (candidates != null && candidates.isArray() && candidates.size() > 0) {
        ...
        if (content != null) {
          ...
          if (parts != null && parts.isArray() && parts.size() > 0) {
            ...
            log.info("[Gemini] Extracted generated text for similar designations: {}",
                generatedText);
            ...
            log.info("[Gemini] Parsed {} designations from 4-pipe separated response",
                designations.size());
            return designations;
          }
        }
      }
      log.error("Unexpected Gemini API response structure for similar designations: {}",
          responseBody);
      return new ArrayList<>();
    } catch (Exception e) {
  ...
  public List<String> generateSimilarCompanies(String companyName, String industry,
    ...
    try {
      log.info(
          "[Gemini] Generating similar companies for: {} in industry: {} with size: {}, specialties: {}, headquarters: {}",
          companyName, industry, companySize, specialties, headquarters);
      String prompt = buildEnhancedSimilarCompaniesPrompt(companyName, industry, description,
          companySize, specialties, headquarters);
      log.info("[Gemini] Enhanced prompt (first 200 chars): {}",
          prompt.length() > 200 ? prompt.substring(0, 200) + "..." : prompt);
      Map<String, Object> requestBody = aiRestService.buildRequestBody(prompt);

      ResponseEntity<String> response = aiRestService.callGeminiAPI(requestBody,
          "Similar Companies for " + companyName);
      List<String> similarCompanies = parseSimilarCompaniesResponse(response.getBody());
      log.info("[Gemini] Parsed {} similar companies for: {}", similarCompanies.size(),
          companyName);
  ...
  private String buildSimilarCompaniesPrompt(String companyName, String industry,
      String description) {
    return buildEnhancedSimilarCompaniesPrompt(companyName, industry, description, null, null,
        null);
  }

  private String buildEnhancedSimilarCompaniesPrompt(String companyName, String industry,
      String description,
      String companySize, String specialties, String headquarters) {
    StringBuilder promptBuilder = new StringBuilder();
    promptBuilder.append(
        "You are an expert career advisor specializing in job transitions and company analysis. ");
    promptBuilder.append(
        "Your task is to find highly relevant similar companies for career transitions.\n\n");
    ...
    promptBuilder.append(
        "2. SIMILAR COMPANY SIZE (startup-to-startup, enterprise-to-enterprise, mid-size-to-mid-size)\n");
    promptBuilder.append(
        "3. COMPARABLE BUSINESS MODEL (B2B, B2C, SaaS, consulting, e-commerce, etc.)\n");
    ...
    promptBuilder.append(
        "- Companies that are direct competitors (unless specifically relevant)\n");
    ...
    promptBuilder.append(
        "- Consider companies that would be realistic career transition targets\n\n");
      ...
      if (candidates != null && candidates.isArray() && candidates.size() > 0) {
        ...
        if (content != null) {
          ...
          if (parts != null && parts.isArray() && parts.size() > 0) {
            ...
            log.info("[Gemini] Parsed {} companies from 4-pipe separated response",
                companies.size());
            return companies;
          }
        }
