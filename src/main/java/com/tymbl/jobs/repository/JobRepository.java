...
public interface JobRepository extends JpaRepository<Job, Long> {
  ...
  List<Job> findActiveJobsByCompanyIdAndTitle(@Param("companyId") Long companyId,
      @Param("title") String title);

  @Query("SELECT j FROM Job j WHERE j.company LIKE %:companyName% AND j.title LIKE %:title% AND j.active = true")
  List<Job> findActiveJobsByCompanyAndTitle(@Param("companyName") String companyName,
      @Param("title") String title);
  ...
  @Query("SELECT j FROM Job j WHERE j.company LIKE %:companyName% AND j.title LIKE %:title% AND j.active = true")
  List<Job> findActiveJobsByCompanyNameAndTitle(@Param("companyName") String companyName,
      @Param("title") String title);
  ...
  List<Job> findByCompanyContainingIgnoreCaseAndTitleContainingIgnoreCase(String companyName,
      String title);
  ...
  Page<Job> searchApprovedJobs(@Param("keyword") String keyword,
      @Param("approved") Integer approved, Pageable pageable);

  @Query("SELECT j FROM Job j WHERE j.active = true AND j.approved = :approved AND " +
      "EXISTS (SELECT s FROM j.skillIds s WHERE s IN :skills)")
  Page<Job> findBySkillsInAndApproved(@Param("skills") Set<Long> skills,
      @Param("approved") Integer approved, Pageable pageable);
  ...
  List<Job> findByCompanyContainingIgnoreCaseAndApproved(@Param("companyName") String companyName,
  ...
  List<Job> findActiveJobsByCompanyIdAndTitleAndApproved(@Param("companyId") Long companyId,
      @Param("title") String title, @Param("approved") Integer approved);
  ...
  List<Job> findActiveJobsByCompanyAndTitleAndApproved(@Param("companyName") String companyName,
      @Param("title") String title, @Param("approved") Integer approved);
