    ...
    try {
      ...
      for (ExternalJobDetail job : unprocessedJobs) {
        try {
          ...
          singleJobContentRefinementService.refineJobContent(job, null); // No designation available for unprocessed jobs
          processedCount++;
  ...
  public boolean refineSpecificJobContent(Long externalJobId) {
    try {
      ...
      if (jobOpt.isPresent()) {
        ...
        // Use the single job refinement service with transaction
        singleJobContentRefinementService.refineJobContent(job, null); // No designation available for specific job refinement
      ...
      log.error("Error refining specific job content for ID {}: {}", externalJobId, e.getMessage(), e);
      throw e;
    }
  }
}
