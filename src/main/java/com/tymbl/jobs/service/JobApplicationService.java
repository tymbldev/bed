...
public class JobApplicationService {
  ...
  @Transactional
  public JobApplicationResponse switchReferrer(Long applicationId, Long newJobReferrerId,
      User applicant) {
    JobApplication application = jobApplicationRepository.findById(applicationId)
        .orElseThrow(() -> new RuntimeException("Application not found"));
    ...
    if (application.getStatus() != JobApplication.ApplicationStatus.PENDING) {
      throw new RuntimeException(
          "You cannot switch referrer after your application is already shared with the other referrer");
    }
  ...
  @Transactional
  public JobApplicationResponse acceptOrRejectApplicationByReferrer(Long jobId, Long applicantId,
      ApplicationStatus status, User referrer) {
    // Find the application by jobId and applicantId
    List<JobApplication> applications = jobApplicationRepository.findByJobIdAndApplicantId(jobId,
        applicantId);
    if (applications.isEmpty()) {
      throw new RuntimeException("Application not found");
    }
    JobApplication application = applications.get(
        0); // Should only be one application per job per applicant
  ...
  public JobApplicationResponse updateApplicationStatus(Long applicationId,
      ApplicationStatus status, User user) {
    JobApplication application = jobApplicationRepository.findById(applicationId)
        .orElseThrow(() -> new RuntimeException("Application not found"));
    ...
    if (!job.getPostedById().equals(user.getId()) && !application.getJobReferrerId()
        .equals(user.getId())) {
   ...
   */
  private void enrichJobApplicationDetailsWithDropdownValues(
      JobApplicationResponseExtendedDetails details) {
    try {
      ...
      if (details.getApplicantDesignationId() != null) {
        String designationName = dropdownService.getDesignationNameById(
            details.getApplicantDesignationId());
      ...
      if (details.getApplicantDepartmentId() != null) {
        String departmentName = dropdownService.getDepartmentNameById(
            details.getApplicantDepartmentId());
      ...
      if (details.getApplicantCurrentSalaryCurrencyId() != null) {
        String currencyName = dropdownService.getCurrencyNameById(
            details.getApplicantCurrentSalaryCurrencyId());
        String currencySymbol = dropdownService.getCurrencySymbolById(
            details.getApplicantCurrentSalaryCurrencyId());
      ...
      if (details.getApplicantExpectedSalaryCurrencyId() != null) {
        String currencyName = dropdownService.getCurrencyNameById(
            details.getApplicantExpectedSalaryCurrencyId());
        String currencySymbol = dropdownService.getCurrencySymbolById(
            details.getApplicantExpectedSalaryCurrencyId());
      ...
      logger.warn(
          "Failed to enrich job application details with dropdown values for application {}: {}",
