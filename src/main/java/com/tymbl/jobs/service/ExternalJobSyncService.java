...
public class ExternalJobSyncService {
  ...
  /**
   * Sync external job details to the main Job table
   * Only processes jobs that haven't been synced yet (isSyncedFromExternal = false)
   * Note: Transaction is handled at the individual job level for better granularity
   */
    ...
    try {
      ...
      for (ExternalJobDetail externalJob : unsyncedJobs) {
        try {
          ...
          IndividualJobSyncService.SyncResult individualResult = individualJobSyncService.syncIndividualJob(externalJob);
          
          if (individualResult.isSuccess()) {
            successCount++;
            log.debug("Successfully synced external job {}: {}", externalJob.getId(), individualResult.getMessage());
          } else {
            errorCount++;
            log.warn("Failed to sync external job {}: {}", externalJob.getId(), individualResult.getMessage());
          }
      ...
      result.setMessage(String.format("Sync completed. Success: %d, Errors: %d", successCount, errorCount));
  ...
  public SyncStatistics getSyncStatistics() {
    ...
    stats.setSyncPercentage(totalExternalJobs > 0 ? (double) syncedJobs / totalExternalJobs * 100 : 0);
  ...
  public static class SyncStatistics {
    ...
    // Getters and setters
    public long getTotalExternalJobs() { return totalExternalJobs; }
    public void setTotalExternalJobs(long totalExternalJobs) { this.totalExternalJobs = totalExternalJobs; }
    
    public long getSyncedJobs() { return syncedJobs; }
    public void setSyncedJobs(long syncedJobs) { this.syncedJobs = syncedJobs; }
    
    public long getUnsyncedJobs() { return unsyncedJobs; }
    public void setUnsyncedJobs(long unsyncedJobs) { this.unsyncedJobs = unsyncedJobs; }
