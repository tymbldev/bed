...
public class UserController {
  ...
  public ResponseEntity<Object> getProfileCompletionStatus(
      @RequestHeader("Authorization") String token) {
    try {
      String email = jwtService.extractUsername(token.substring(7));
      User user = registrationService.getUserByEmail(email);
      ...
      logger.debug("Profile completion for user {}: {}%", user.getEmail(),
  ...
  private ProfileCompletionResponse calculateProfileCompletion(User user) {
    ...
    if (user.getYearsOfExperience() == null) {
      pendingFields.add(createPendingField("yearsOfExperience", "Years of Experience",
    ...
    if (user.getMonthsOfExperience() == null) {
      pendingFields.add(createPendingField("monthsOfExperience", "Months of Experience",
          "Add your months of experience"));
    } else {
      completedFields++;
    }
    ...
    if (user.getCurrentSalary() == null) {
      pendingFields.add(
          createPendingField("currentSalary", "Current Salary", "Add your current salary"));
    } else {
      completedFields++;
    }
    ...
    if (user.getExpectedSalary() == null) {
      pendingFields.add(
          createPendingField("expectedSalary", "Expected Salary", "Add your expected salary"));
    } else {
      completedFields++;
    }
    ...
    if (user.getNoticePeriod() == null) {
      pendingFields.add(
          createPendingField("noticePeriod", "Notice Period", "Add your notice period"));
    } else {
      completedFields++;
    }
    ...
    if (user.getLinkedInProfile() == null || user.getLinkedInProfile().trim().isEmpty()) {
      pendingFields.add(createPendingField("linkedInProfile", "LinkedIn Profile",
    ...
    if (user.getGithubProfile() == null || user.getGithubProfile().trim().isEmpty()) {
      pendingFields.add(
          createPendingField("githubProfile", "GitHub Profile", "Add your GitHub profile URL"));
    } else {
      completedFields++;
    }

    if (user.getPortfolioWebsite() == null || user.getPortfolioWebsite().trim().isEmpty()) {
      pendingFields.add(createPendingField("portfolioWebsite", "Portfolio Website",
